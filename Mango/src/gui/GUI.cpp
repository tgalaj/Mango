...
namespace mango
{
    ...
    void GUI::image(std::shared_ptr<Texture> pTexture, const glm::vec2& from, const glm::vec2& to,  const glm::vec4& color)
    {
        ...
        const auto tint_color = glm::clamp(color, 0.0f, 1.0f);

        window->DrawList->AddImage(reinterpret_cast<void*>(pTexture->m_to_id), { from.x, from.y }, { to.x, to.y }, { 0.0f, 0.0f }, { 1.0f, 1.0f }, ImGui::GetColorU32({ tint_color.r, tint_color.g, tint_color.b, tint_color.a }));
    }
    ...
    void GUI::imageRounded(std::shared_ptr<Texture> pTexture, const glm::vec2& from, const glm::vec2& to,  const glm::vec4& color, float rounding, uint32_t roundingCornersFlags)
    {
        ...
        window->DrawList->AddImageRounded(reinterpret_cast<void*>(pTexture->m_to_id), { from.x, from.y }, { to.x, to.y }, { 0.0f, 0.0f }, { 1.0f, 1.0f }, ImGui::GetColorU32({ tint_color.r, tint_color.g, tint_color.b, tint_color.a }), rounding, roundingCornersFlags);
    }
}
